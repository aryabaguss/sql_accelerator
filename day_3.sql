...
select distinct 
		...
		d.dept_name ,
		e.first_name ,
		--(t.to_date - t.from_date) / 365 as years,
		count(e.first_name)
	from title t 
	...
	group by de.department_id ,d.dept_name ,e.first_name 
	having count(e.first_name) > 1
	order by de.department_id ,e.first_name 
...
WITH temp AS (
    ...
    JOIN department_employee de ON de.employee_id = e.id
    JOIN department d ON d.id = de.department_id
    GROUP BY de.department_id, d.dept_name, e.id, e.first_name
    HAVING COUNT(*) > 1  -- This ensures multiple entries for the same employee
    ORDER BY de.department_id, e.first_name
),
totall AS (
    SELECT
        de.department_id,
        d.dept_name,
        AVG((t.to_date - t.from_date) / 365.0) AS avg_years
    FROM title t
    JOIN employee e ON e.id = t.employee_id
    JOIN department_employee de ON de.employee_id = e.id
    JOIN department d ON d.id = de.department_id
